...
public class Vliegtuig {
	...
	private static final int MAX_PASSAGIERS 	= 199; // Het maximaal aantal passagiers dat het vliegtuig kan vervoeren
	private static final int MAX_BEREIK 		= 3199;// Het aantal kilometers dat het vliegtuig kan afleggen op 1 tank
	private static final int VLIEGTUIG_SNELHEID = 800; // Maximale snelheid van het vliegtuig in km/h
	private int aantalLandingen; 					   // De route die het vliegtuig aflegt: een rijtje landingen
	...
	public Landing geefLanding(int index) {
		if (index > aantalLandingen - 1) {
			System.out.println("geefLanding: voor index " + index + " is er geen landing");
			return null;
		}
		return route[index];
	}
	...
	// lucht droog komt te staan wanneer dit niet gebeurt. 
	// !! GAAT ER VAN UIT DAT ER NOG NIET GETANKT WORDT TIJDENS EEN LANDING
	public void planTankbeurten() {
		if (aantalLandingen > 1) {
			int bereik = MAX_BEREIK;
			for (int i = 1; i < aantalLandingen; i++) {
				int afstand = geefLanding(i - 1).geefAfstandNaar(geefLanding(i).geefLoc());
				if (afstand > MAX_BEREIK) {
					System.out.println("Een vliegtuig vliegt naar een bestemming die buiten zijn bereik ligt");
				}
				if (afstand > bereik) {
	...
	// Retouneert de verzameling landingen van dit vliegtuig vanaf beginIndex met lengte
	public Landing[] geefSubroute(int beginIndex, int lengte) {
		...
		System.out.println("Een subroute met lengte " + lengte + " is te groot vanaf " + beginIndex);
		return null; // Een subroute met deze lengte is te groot vanaf beginIndex
	}
	...
	public boolean voegRouteToe(Landing[] toeTeVoegen) {
		...
		if (nieuweLengte > MAX_ROUTE_LENGTE) {
			System.out.print("CombineerRoutes: lengte van samengevoegde route is te groot ");
			System.out.println("(" + this.aantalLandingen + " + " + toeTeVoegen.length + " > " + MAX_ROUTE_LENGTE + ")");
			return false;
		}
	...
	public boolean wijzigLanding(int index, City nieuweLoc, int nieuweGrondtijd, boolean hierTanken) {
		// <= : een wijziging mag de eerstvolgende lege index zijn of een bestaande index. 
		// Maar nooit groter dan de lengte van de route
		if (index <= aantalLandingen && index < route.length) {
			route[index] = new Landing(nieuweLoc, nieuweGrondtijd, hierTanken);
			return true;
		}

		System.out.println("WijzigLanding: meegegeven index " + index + "is te groot. (" + aantalLandingen + "/" + route.length + ")");
		return false;
	}
	...
	public boolean voegLandingToe(City nieuweLoc, int nieuweGrondtijd, boolean hierTanken) {
		if (aantalLandingen < route.length) {
			route[aantalLandingen] = new Landing(nieuweLoc, nieuweGrondtijd, hierTanken);
			aantalLandingen++;
			return true;
		}
	...
	public boolean verwijderLanding(int index) {
		if (index < route.length) {
			aantalLandingen--; // door dit hier te zetten krijgen we geen array out of bounds in de komende for loop
		...
		System.out.println("verwijderLanding: te grote index (" + index + "/" + (route.length - 1) + ")");
		return false;
	}
	...
	public boolean insertLanding(int index, Landing landing) {
		...
		System.out.println("insertLanding: te grote index (" + index + "/" + (route.length - 1) + ")");
		return false;
	}

	// Maakt een array met vertrektijden van dit vliegtuig
	// Het geeft terug een array van opstijgingen met de info:
	// 1) tijdstip van vertrek 2) vertreklocatie 3) bestemming 4) vliegtuig capaciteit
	public int[][] maakVertrekArray() {
		// Als het vliegtuig opstijgt
		if (aantalLandingen > 1) {
			int time = route[0].geefGrondtijd(); // niet totale grondtijd want het is het eerste punt op de route
			int[][] result = { { time, route[0].geefLoc(), route[1].geefLoc(), MAX_PASSAGIERS } };

			for (int i = 2; i < aantalLandingen; i++) {
				Landing a = route[i - 1];
				Landing b = route[i];
				double tmp = (double) a.geefAfstandNaar(b.geefLoc()) / VLIEGTUIG_SNELHEID * 60; // *60 om per min ipv uur te krijgen
				time += (int) tmp + b.geefTotaleGrondtijd();
				int[][] array = { { time, a.geefLoc(), b.geefLoc(), MAX_PASSAGIERS } };
